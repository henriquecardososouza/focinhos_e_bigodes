...
@section("content")
    ...
    <main class="my-12">
        <h1 class="font-dosis text-brown-500 text-4xl mb-8 font-bold">Editar dados da raça de pet {{ $raca->nome }}</h1>

        <div class="flex flex-col justify-center align-center p-6 rounded-xl shadow-[0_0_20px_#00000055] max-w-[400px] w-[95%] mx-auto">
            <h1 class="text-center font-red-hat text-xl mb-4">Editar raça</h1>
            <div class="flex flex-col gap-[5px] my-2">
                <label for="nome">Nome</label>
                <input disabled class="border-1 border-orange-600 rounded-xl transition duration-400 py-2 px-3 focus:border-orange-800 focus:ring-0 focus:outline-0" type="text" id="nome" name="nome" placeholder="Ex.: Pastro alemão" value="{{ $raca->nome }}">
            </div>
            ...
            <div class="flex flex-col gap-[5px] my-2">
                ...
                <select class="border-1 border-orange-600 rounded-xl transition duration-400 py-2 px-3 focus:border-orange-800 focus:ring-0 focus:outline-0" id="tipo" name="tipo">
                    @foreach(\App\Models\TipoPet::orderBy("nome", "ASC")->get() as $tipo)
                        <option value="{{ $tipo->nome }}" @if ($raca->tipo === $tipo->nome) selected @endif>{{ $tipo->nome }}</option>
                    @endforeach
                </select>
            </div>
...
@push('script')
    <script>
        $("#btn-editar").on("click", function (e) {
            $.ajax({
                url: "{{ route("intern.raca.save", $raca->nome) }}",
                method: "POST",
                data: {
                    nome: $("#nome").val(),
                    tipo: $("#tipo").val(),
                ...
                success: function (res) {
                    showToastr("success", "Raça atualizada", res.message);
